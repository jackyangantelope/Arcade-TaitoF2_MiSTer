...
struct ObjCacheEntry
{
    ...
    ObjCacheEntry(SDL_Texture *tex) : texture(tex), last_used(0)
    {
    }
...
SDL_Texture *get_obj_texture(uint16_t code, uint8_t palette)
{
    ...
    for (int i = 0; i < 16; i++)
    {
        rawpal[i] = (s_palette_high[pal_ofs + i] << 8) |
                    (s_palette_low[pal_ofs + i] << 0);
    }
    ...
    SDL_Texture *tex = SDL_CreateTexture(s_renderer, SDL_PIXELFORMAT_RGBX8888,
...
void draw_obj_window()
{
                ...
                ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_RowBg))
    {
        ...
        if (tooltip_idx >= 0)
        {
            ...
            {
                SDL_Texture *tex =
                    get_obj_texture(code, latched_color[tooltip_idx]);
                ImGui::BeginTooltip();
                              ...
                              ImGuiTableFlags_SizingFixedFit))
    {
        ...
        while (clipper.Step())
        {
                 ...
                 index < clipper.DisplayEnd; index++)
            {
                ...
                {
                    ImGui::TableNextColumn();
                    SDL_Texture *tex = get_obj_texture((uint16_t)base_code + i,
                                                       (uint8_t)color);
